...
inline s32 getAverage(u8 color)
{
...
stored in a single UInt32. So all four values may be between 0 and 255.
...
This class must consist of only one UInt32 and must not use virtual functions.
*/
class QcColor
{
public:
	...
	QcColor() : color(0xFFFFFFFF){}

	//! Constructs the color from 4 values representing the alpha, red, green and blue component.
	/** Must be values between 0 and 255. */
	QcColor (u32 a, u32 r, u32 g, u32 b)
	...
	D3DCOLORVALUE& ToD3DColorValue(D3DCOLORVALUE& value) const
	{
		const f32 inv = 1.0f / 255.0f;
		value.r = getRed() * inv;
		value.g = getGreen() * inv;
		value.b = getBlue() * inv;
		value.a = getAlpha() * inv;
		return value;
	}
	...
	bool operator==(const QcColor& other) const { return other.color == color; }
	...
	bool operator<(const QcColor& other) const { return (color < other.color); }

	//! Adds two colors, result is clamped to 0..255 values
	/** \param other Color to add to this color
	\return Addition of the two colors, clamped to 0..255 values */
	QcColor operator+(const QcColor& other) const
	{
		return QcColor(min_(getAlpha() + other.getAlpha(), u32(255u)),
			min_(getRed() + other.getRed(), u32(255u)),
			min_(getGreen() + other.getGreen(), u32(255u)),
			min_(getBlue() + other.getBlue(), u32(255u)));
	}
	...
	QcColor getInterpolated(const QcColor &other, f32 d) const
	{
		d = clamp(d, 0.f, 1.f);
		const f32 inv = 1.0f - d;
		return QcColor((u32)round32(other.getAlpha()*inv + getAlpha()*d),
			(u32)round32(other.getRed()*inv + getRed()*d),
			(u32)round32(other.getGreen()*inv + getGreen()*d),
			(u32)round32(other.getBlue()*inv + getBlue()*d));
	}
	...
	\param d: value between 0.0f and 1.0f. */
	QcColor getInterpolated_quadratic(const QcColor& c1, const QcColor& c2, f32 d) const
	{
...
public:
	...
	QcColorf() : r(0.0f), g(0.0f), b(0.0f), a(1.0f) {}
	...
	QcColorf(f32 r, f32 g, f32 b, f32 a = 1.0f) : r(r), g(g), b(b), a(a) {}
	...
	QcColor toSColor() const
	{
		return QcColor((u32)round32(a*255.0f), (u32)round32(r*255.0f), (u32)round32(g*255.0f), (u32)round32(b*255.0f));
	}
	...
	\param d: value between 0.0f and 1.0f
	\return Interpolated color. */
	QcColorf getInterpolated(const QcColorf &other, f32 d) const
	{
		d = clamp(d, 0.f, 1.f);
		const f32 inv = 1.0f - d;
		return QcColorf(other.r*inv + r*d,
	...
	\param c2: second color to interpolate with
	\param d: value between 0.0f and 1.0f. */
	inline QcColorf getInterpolated_quadratic(const QcColorf& c1, const QcColorf& c2,
		f32 d) const
	{
		d = clamp(d, 0.f, 1.f);
		// this*(1-d)*(1-d) + 2 * c1 * (1-d) + c2 * d * d;
		...
		return QcColorf (r * mul0 + c1.r * mul1 + c2.r * mul2,
...
{
public:
	QcColorHSL ( f32 h = 0.f, f32 s = 0.f, f32 l = 0.f )
		: Hue ( h ), Saturation ( s ), Luminance ( l ) {}

	void fromRGB(const QcColor &color);
	void toRGB(QcColor &color) const;
...
inline void QcColorHSL::fromRGB(const QcColor &color)
{
...
inline void QcColorHSL::toRGB(QcColor &color) const
{
...
inline u32 QcColorHSL::toRGB1(f32 rm1, f32 rm2, f32 rh) const
{
	while ( rh > 2.f * PI )
		rh -= 2.f * PI;
	...
	return (u32) round32(rm1 * 255.f);
}

template<>
inline QcColor StrToValue<QcColor>(const char* psStr)
{
	u32 ar[4] = {255,255,255,255};
	StrToValueList(psStr, ar, ',');
	return QcColor(ar[3], ar[0], ar[1], ar[2]);
}
template<>
inline QcColorf StrToValue<QcColorf>(const char* psStr)
{
	f32 ar[4] = {1.f, 1.f, 1.f, 1.f};
	StrToValueList(psStr, ar, ',');
	return QcColorf(ar[3], ar[0], ar[1], ar[2]);
}