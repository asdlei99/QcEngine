...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTCreateDevice( bool bWindowed, int nSuggestedWidth, int nSuggestedHeight, D3DFORMAT nSuggestedDepthStencilFormat)
{
    HRESULT hr = S_OK;
    ...
    matchOptions.eDepthFormat = /*DXUTMT_IGNORE_INPUT*/DXUTMT_PRESERVE_INPUT;
    matchOptions.eStencilFormat = /*DXUTMT_IGNORE_INPUT*/DXUTMT_PRESERVE_INPUT;
	...
	deviceSettings.d3d9.pp.AutoDepthStencilFormat = nSuggestedDepthStencilFormat;
...
void DXUTRender3DEnvironment9()
{
    ...
    if( GetDXUTState().GetDeviceLost() && !GetDXUTState().GetRenderingPaused() )
    {
        ...
        if( FAILED( hr = pd3dDevice->TestCooperativeLevel() ) )
        {
			...
			if (hr == D3DERR_DEVICENOTRESET)
			{
				...
				if (nCount > 200)
				{
					DXUTDisplayErrorMessage(D3DERR_DEVICENOTRESET);
				}
				return;
			}
			else
	...
	if (fElapsedTime > 1.f/20.f)
		fElapsedTime = 1.f/20.f;
...
void DXUTDisplayErrorMessage( HRESULT hr )
{
    ...
    switch( hr )
    {
		...
		case D3DERR_DEVICELOST:
			nExitCode = 12; wcscpy_s( strBuffer, ARRAYSIZE(strBuffer), L"D3DERR_DEVICELOST" );
			break;
        default:
...
void DXUTUpdateFrameStats()
{
    ...
    if( fAbsTime - fLastTime > 1.0f )
    {
        ...
        swprintf_s( pstrFPS, 64, L"%0.2f fps \n", fFPS );

		//OutputDebugStringW(pstrFPS);
    }
}
