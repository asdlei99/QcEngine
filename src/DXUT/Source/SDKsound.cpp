...
HRESULT CSoundManager::Create( CSound** ppSound, BYTE* pbData, ULONG ulDataSize, DWORD dwCreationFlags,
							   GUID guid3DAlgorithm, DWORD dwNumBuffers)
{
	HRESULT hr;
	HRESULT hrRet = S_OK;
	DWORD i;
	...
	pWaveFile->Open(pbData, ulDataSize);
...
#if 1
	++m_index;
	if (m_index == m_dwNumBuffers)
		m_index = 0;
	m_apDSBuffer[m_index]->SetCurrentPosition(0);
	return m_apDSBuffer[m_index];
#else
    DWORD i;
