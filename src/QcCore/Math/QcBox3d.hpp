...
public:
	...
	//! Default Constructor.
	QcBox3d(): MinEdge(-1,-1,-1), MaxEdge(1,1,1) {}
	//! Constructor with min edge and max edge.
	QcBox3d(const QcVector3d<T>& min, const QcVector3d<T>& max): MinEdge(min), MaxEdge(max) {}
	//! Constructor with only one point.
	QcBox3d(const QcVector3d<T>& init): MinEdge(init), MaxEdge(init) {}
	//! Constructor with min edge and max edge as single values, not vectors.
	QcBox3d(T minx, T miny, T minz, T maxx, T maxy, T maxz): MinEdge(minx, miny, minz), MaxEdge(maxx, maxy, maxz) {}

	// operators
	//! Equality operator
	/** \param other box to compare with.
	\return True if both boxes are equal, else false. */
	inline bool operator==(const QcBox3d<T>& other) const { return (MinEdge == other.MinEdge && other.MaxEdge == MaxEdge);}
	//! Inequality operator
	/** \param other box to compare with.
	...
	void reset(const QcBox3d<T>& initValue)
	{
		*this = initValue;
	}
	...
	void reset(const QcVector3d<T>& initValue)
	{
		MaxEdge = initValue;
		MinEdge = initValue;
	}
	...
	void addInternalPoint(const QcVector3d<T>& p)
	{
	...
	void addInternalBox(const QcBox3d<T>& b)
	{
	...
	T getVolume() const
	{
		const QcVector3d<T> e = getExtent();
		return e.X * e.Y * e.Z;
	}
	...
	void getEdges(QcVector3d<T> *edges) const
	{
		const QcVector3d<T> middle = getCenter();
		const QcVector3d<T> diag = middle - MaxEdge;
	...
	QcBox3d<T> getInterpolated(const QcBox3d<T>& other, f32 d) const
	{
		f32 inv = 1.0f - d;
		return QcBox3d<T>((other.MinEdge*inv) + (MinEdge*d),
	...
	bool isPointInside(const QcVector3d<T>& p) const
	{
	...
	\param p: Point to check.
	\return True if the point is within the box and false if not. */
	bool isPointTotalInside(const QcVector3d<T>& p) const
	{
		return (p.X > MinEdge.X && p.X < MaxEdge.X &&
			p.Y > MinEdge.Y && p.Y < MaxEdge.Y &&
			p.Z > MinEdge.Z && p.Z < MaxEdge.Z);
	...
	otherwise false. */
	bool isFullInside(const QcBox3d<T>& other) const
	{
	...
	otherwise false. */
	bool intersectsWithBox(const QcBox3d<T>& other) const
	{
	...
	bool intersectsWithLine(const QcLine3d<T>& line) const
	{
	...
	\return True if there is an intersection, else false. */
	bool intersectsWithLine(const QcVector3d<T>& linemiddle,
		const QcVector3d<T>& linevect, T halflength) const
	{
		const QcVector3d<T> e = getExtent() * (T)0.5;
		const QcVector3d<T> t = getCenter() - linemiddle;
	...
	EIntersectionRelation3D classifyPlaneRelation(const QcPlane3d<T>& plane) const
	{
		QcVector3d<T> nearPoint(MaxEdge);
		QcVector3d<T> farPoint(MinEdge);
	...
	//! The near edge
	QcVector3d<T> MinEdge;

	//! The far edge
	QcVector3d<T> MaxEdge;
};

//! Typedef for a f32 3d bounding box.
typedef QcBox3d<f32> QcBox3df;
//! Typedef for an integer 3d bounding box.
typedef QcBox3d<s32> QcBox3dfi;
