...
#pragma message(QmToStr(_WIN32_WINNT))
...
bool VisualLeakDetector::waitForAllVLDThreads()
{
    ...
    for (TlsMap::Iterator tlsit = m_tlsMap->begin(); tlsit != m_tlsMap->end(); ++tlsit) {
		...
		DWORD dwThreadId = (*tlsit).second->threadId;
        HANDLE thread = OpenThread(SYNCHRONIZE | THREAD_QUERY_INFORMATION, FALSE, dwThreadId);
        if (thread == NULL) {
